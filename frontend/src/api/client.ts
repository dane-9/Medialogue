...
export const api = {
  ...
  testDownloadClient: (id?: string, configuration?: { name: string; url: string; username?: string; password?: string; scope: DownloadClientScope; category?: string; tags: string[]; enabled: boolean; poll_interval_seconds?: number }) => request<DownloadClientTestResult>(id ? `/api/v1/download-clients/${encodeURIComponent(id)}/test` : '/api/v1/download-clients/test', { method: 'POST', ...(configuration ? { body: JSON.stringify(configuration) } : {}) }),
